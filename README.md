...
flowchart LR
  ...
  %% =========================
  subgraph I[infra (implementations)]
    MYSQL[(MySQL 5.7)]
    REDIS[(Redis)]
    KAFKA[(Kafka)]
  end
  ...
  %% 실제依赖表达(理解用)
  APP -. uses repo/cache/outbox ports .-> I
  CONSUMER -. uses audit/idempotency .-> I

  %% =========================
  %% Main flow: UserCreated
  %% =========================
  API -->|POST /users| APP
  APP -->|Tx: insert| USERS
  APP -->|Tx: insert event| OUTBOX

  DISPATCH -->|poll unsent| OUTBOX
  DISPATCH -->|publish UserCreated| KAFKA

  CONSUMER -->|consume user.events| KAFKA
  CONSUMER -->|SETNX inbox:*| REDIS
  CONSUMER --> AUDITAPP
  AUDITAPP -->|insert| AUDIT
